...
# ---------------------------------------------
def hands_funcion(puerto_com, camara, cantidad_servo, get_ports, active_arduino):  
    mp_dibujo = mp.solutions.drawing_utils
    mp_manos = mp.solutions.hands
    cap = cv2.VideoCapture(int(camara))

    try:
        if active_arduino == "on":
            arduino = serial.Serial(puerto_com, 9600)
            print("Conexión con Arduino establecida.")
        else:
            arduino = None  # Simular que no hay comunicación con Arduino
